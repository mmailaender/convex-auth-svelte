...
export function createAuthClient({
		...
		const loadTokens = async () => {
			...
			if (serverState?._state) {
				...
				if (serverIsNewer) {
					...
					await setToken({
						shouldStore: true,
						tokens: token === null || refreshToken === null ? null : { token, refreshToken }
					});
				} else {
	...
	$effect(() => {
		const checkUrlForCode = async () => {
			...
			if (code) {
				...
				await replaceURL(url.pathname + url.search + (url.hash ? url.hash : ''));
	...
	const signIn = async (provider: string, params?: FormData | Record<string, Value>) => {
		...
		try {
			...
			const finalParams =
				params instanceof FormData ? Object.fromEntries(params.entries()) : (params ?? {});

			const result = await client.authenticatedCall('auth:signIn' as unknown as SignInAction, {
				provider,
				params: finalParams,
				verifier
			});
	...
	const signOut = async (): Promise<void> => {
		...
		try {
			...
			await client.authenticatedCall('auth:signOut' as unknown as SignOutAction);
		} catch (e) {
...
export async function browserMutex<T>(key: string, callback: () => Promise<T>): Promise<T> {
	if (typeof window === 'undefined') {
		return callback();
	}
...
export const setupConvexClient = (convexUrl: string, options?: ConvexClientOptions) => {
